...
const InPageSearch: React.FC<InPageSearchProps> = ({ htmlContent }) => {
  ...
  console.log(
    "%c[InPageSearch] Componente inicializado",
  ...
  const removeHighlights = useCallback(() => {
    console.log(
      "%c[InPageSearch] Removendo highlights anteriores",
      "color: #FF9800"
    );
  ...
  const highlightText = useCallback((text: string, term: string): string => {
    ...
    return text.replace(
      regex,
      '<mark class="in-page-search-highlight bg-yellow-300 text-black px-1 rounded">$1</mark>'
    );
  }, []);
  ...
  const performSearch = useCallback(
    (term: string) => {
      console.log(
        `%c[InPageSearch] Realizando busca por: "${term}"`,
        "color: #2196F3; font-weight: bold"
      );
      setIsSearching(true);
      removeHighlights();
      ...
      if (!contentElement) {
        console.warn(
          "%c[InPageSearch] Elemento .markdown-body não encontrado",
          "color: #FF9800"
        );
        setIsSearching(false);
        return;
      ...
      console.log(
        `%c[InPageSearch] Encontrados ${foundMatches.length} resultados`,
  ...
  const navigateToMatch = useCallback(
    (index: number, matchList: SearchMatch[] = matches) => {
      if (index < 0 || index >= matchList.length) return;

      console.log(
        `%c[InPageSearch] Navegando para resultado ${index + 1}/${
          matchList.length
        }`,
        "color: #2196F3"
      );
      ...
      const highlightElements = currentMatch.element.querySelectorAll(
        ".in-page-search-highlight"
      );
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        console.log(
          "%c[InPageSearch] Clique fora detectado, fechando busca",
          "color: #F44336"
        );
        closeSearch();
      }
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <input
          ...
          className="flex-1 px-2 py-1 text-sm border border-gray-300 dark:border-gray-600 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400 transition-colors"
        />
        <button
          onClick={closeSearch}
          className="p-1 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none transition-colors"
          title="Fechar (Esc)"
        >
          <X className="w-4 h-4" />
        </button>
          ...
          {matches.length > 0 && (
            <div className="flex items-center gap-1">
              <button
                ...
                className="p-1 text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 focus:outline-none disabled:opacity-50 transition-colors"
      ...
      {matches.length > 0 && (
        <div className="in-page-search-results-container mt-2 max-h-60 overflow-y-auto border-t border-gray-200 dark:border-gray-600 pt-2 transition-colors">
          {matches.map((match, index) => (
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: highlightText(match.context, searchTerm),
              }}
            />
          ))}
        </div>
